...
func main() {
    ...
    useNTLM := flag.Bool("use-ntlm", false, "Enable NTLM authentication")
    username := flag.String("username", "", "Username for NTLM authentication (e.g., domain\\user)")
    password := flag.String("password", "", "Password for NTLM authentication")
    ...
    // Initialize custom HTTP client with NTLM support and credentials
    customHTTPClient = createCustomHTTPClient(*userAgent, !*validate, *httpTimeout, *useNTLM, *username, *password)
